...
const Sidebar = () => {
    ...
    const menuItems = {
        admin: [
            ...
            { path: '/logistics', label: 'Delivery Map', icon: <Truck size={20} /> },
        ]
    };
        ...
        <aside style={{
            ...
            <div style={{ 
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: '0.9rem', fontWeight: 800, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{user?.name || 'Admin'}</p>
                    <p style={{ fontSize: '0.75rem', color: '#94a3b8', textTransform: 'capitalize', fontWeight: 600 }}>{user?.role || 'admin'}</p>
                </div>
            </div>
        </aside>
    );
